...
void ChatBot::ReceiveMessageFromUser(std::string message) {
  ...
  if (!levDists.empty()) {
    ...
    newNode = levDists.at(
      0).first->GetChildNode(); // after sorting the best edge is at first position
  } else {
...
int ChatBot::ComputeLevenshteinDistance(std::string s1, std::string s2) {
  ...
  size_t i = 0;
  for (std::string::const_iterator it1 = s1.begin();
       it1 != s1.end(); ++it1, ++i) {
    costs[0] = i + 1;
    size_t corner = i;

    size_t j = 0;
    for (std::string::const_iterator it2 = s2.begin();
         it2 != s2.end(); ++it2, ++j) {

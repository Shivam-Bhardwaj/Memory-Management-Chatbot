...
ChatBotFrameImagePanel::ChatBotFrameImagePanel(wxFrame *parent) : wxPanel(
  parent) {
}
...
void ChatBotPanelDialog::AddDialogItem(wxString text, bool isFromUser) {
  ...
  ChatBotPanelDialogItem *item = new ChatBotPanelDialogItem(this, text,
                                                            isFromUser);
  _dialogSizer->Add(item, 0,
                    wxALL | (isFromUser == true ? wxALIGN_LEFT : wxALIGN_RIGHT),
                    8);
  _dialogSizer->Layout();
...
ChatBotPanelDialogItem::ChatBotPanelDialogItem(wxPanel *parent, wxString text,
  ...
  _chatBotImg = new wxStaticBitmap(this, wxID_ANY, (isFromUser ? wxBitmap(
    imgBasePath + "user.png", wxBITMAP_TYPE_PNG) : *bitmap), wxPoint(-1, -1),
                                   wxSize(-1, -1));
  _chatBotTxt = new wxStaticText(this, wxID_ANY, text, wxPoint(-1, -1),
                                 wxSize(150, -1),
                                 wxALIGN_CENTRE | wxBORDER_NONE);
  _chatBotTxt->SetForegroundColour(
    isFromUser == true ? wxColor(*wxBLACK) : wxColor(*wxWHITE));
